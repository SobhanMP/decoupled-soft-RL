...
class SoftQLearning:
    def __init__(
        ...
        self.n_step = hps["nstep"]
        self.loss = hps["sql_loss"]
        self.exp = hps["sql_exp"]
        ...
        if self.loss == "huber":
            ...
            raise ValueError(f"Unknown loss function {cfg.algo.loss_fn}")
        # Experimental flags
        self.sample_temp = 1
    ...
    def compute_batch_losses(
        ...
        Q_t: GraphActionCategorical
        Q, _ = model(batch, cond_info[batch_idx])
        if target_model is None or target_model is model:
            Q_t = Q.clone()
        else:
            Q_t, _ = target_model(batch, cond_info[batch_idx])

        first_v, shifted_V_soft = self.tree_backup(batch, rewards, final_graph_idx, Q_t.clone())
        v, m = torch.var_mean(shifted_V_soft - first_v)
        ...
        losses = self.loss_fn(Q_sa, shifted_V_soft.detach(), reduction="none")
        ...
        info = {
            ...
            "mean_target": torch.mean(shifted_V_soft),
