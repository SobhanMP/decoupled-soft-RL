...
class MovingMean:
    ...
    def add(self, m, n=1):
        n1 = self.n + n
        self.m = self.m * self.n / n1 + m * n / n1
        self.m2 = self.m2 * self.n / n1 + m * m * n / n1
        self.n = n1
...
class FragMolBuildingEnvContext(GraphBuildingEnvContext):
    ...
    def __init__(
        ...
        self.bench = defaultdict(MovingMean)
    ...
    def graph_to_Data(self, g: Graph) -> gd.Data:
        ...
        stop_mask = (
            torch.zeros((1, 1)) if has_unfilled_attach or not len(g) or len(g) < self.min_frags else torch.ones((1, 1))
        )
        ...
        self.bench[len(g)].add(time.time() - start_time)
        return ret
