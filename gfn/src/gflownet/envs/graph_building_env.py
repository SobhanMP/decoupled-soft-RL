...
def graph_without_node(g: Graph, n):
    if n + 1 == g.number_of_nodes():
        gp = g.copy()
        gp.remove_node(n)
        return gp
    else:
        ...
        for k, v in g.nodes.items():
            if k == n:
                continue
            gp.add_node(k - (k > n), **v)
        for (src, dst), v in g.edges.items():
            if src == n or dst == n:
                continue
            src -= src > n
            dst -= dst > n
            gp.add_edge(src, dst, **v)
        return gp
...
class GraphActionCategorical:
    def __init__(
        ...
        self.types = types
        self.keys = keys
        self.decoupled_entropy = decoupled_entropy
    ...
    def _compute_batchwise_max(
        ...
        min_val = torch.stack([i.min() for i in x if i.numel()], -1).min()
        outs = [torch.full((self.num_graphs, i.shape[1]), min_val, device=self.dev) for i in x]
    ...
    def scat(self, x, b, reduce="sum", out=None):
        return scatter(x, b, dim=0, dim_size=self.num_graphs, reduce=reduce, out=out)

    def get_temp(self, x=None):
        if self.decoupled_entropy:
            n = self.n_actions(x)
            n = n.float().log().clamp(1)
            return [self.temp / n[b, None] for b in self.batch], self.temp / n
        else:
            return [self.temp for _ in self.batch], self.temp
    ...
    def logsoftmax(self):
        ...
        temp, _ = self.get_temp(self.logits)
        logits = [i / t for i, t in zip(self.logits, temp)]
        # Use the `subtract by max` trick to avoid precision errors.
        maxl = self._compute_batchwise_max(logits).values
        # substract by max then take exp
        # x[b, None] indexes by the batch to map back to each node/edge and adds a broadcast dim
        corr_logits = [(i - maxl[b, None]) for i, b in zip(logits, self.batch)]
    ...
    def n_actions(self, x=None):
        if x is None:
            x = self.logits
        return sum(
            [self.scat(m.broadcast_to(i.shape).int().sum(1), b) for i, m, b in zip(x, self.masks, self.batch)],
        )

    def get_n(self, x=None):
        if x is None or x is self.logit:
            if self.n is None:
                self.n = self.n_actions()
            return self.n
        return self.n_actions(x)
    ...
    def logsumexp(self, x=None):
        ...
        # substract by max
        # x[b, None] indexes by the batch to map back to each node/edge and adds a broadcast dim
        x = [(i - maxl[b, None]) for i, b in zip(x, self.batch)]

        temp, rtemp = self.get_temp(x)
        x = [i / t for i, t in zip(x, temp)]
        # take exp
        exp_vals = [i.exp() for i in x]
        # sum corrected exponentiated logits, to get log(Z - max) = log(sum(exp(logits)) - max)
        reduction = sum([self.scat(i, b).sum(1) for i, b in zip(exp_vals, self.batch)])
        # Add back max
        reduction = rtemp * reduction.log() + maxl
    ...
    def sample(self) -> List[Tuple[int, int, int]]:
        ...
        t, _ = self.get_temp(self.logits)
        logits = [logit / temp for logit, temp in zip(self.logits, t)]
        # Uniform noise
        u = [torch.rand(i.shape, device=self.dev) for i in self.logits]
        # Gumbel noise
        gumbel = [logit - (-noise.log()).log() for logit, noise in zip(logits, u)]
        # Take the argmax
    ...
    def calc_offset(self, actions, logprobs, batch):
        ...
        assert [i.shape for i in logprobs] == [i.shape for i in self.logits]
    ...
    def inv_log_prob(
        ...
        cache: Union[bool, List[torch.Tensor]] = False,
    ) -> List[torch.Tensor]:
        ...
        l = sum(i.numel() for i in self.logits)
        if backbone is not None:
            backbone = backbone.clone()
        elif original is not None:
            backbone = torch.cat([i.flatten() for i in original])
        elif full is not None:
            backbone = torch.full((l,), full, device=self.dev, dtype=dtype)
        else:
            backbone = torch.zeros((l,), device=self.dev, dtype=dtype)

        if isinstance(cache, bool):
            t_offsets, row_offsets, col_offsets = self.calc_offset(actions, logits, batch)
        else:
            t_offsets, row_offsets, col_offsets = cache
        ...
        for i in logits:
            j = i.numel()
            x.append(backbone[a : a + j].reshape(i.shape))
            a += j
        assert a == backbone.numel()
        if not isinstance(cache, bool) or cache:
            return x, [t_offsets, row_offsets, col_offsets]
        else:
            return x
