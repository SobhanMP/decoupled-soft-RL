...
class SamplingIterator(IterableDataset):
    ...
    def __iter__(self):
        ...
        for idcs in self._idx_iterator():
            ...
            rp = self.random_action_prob.value
            # Sample some on-policy data
            if num_online > 0:
                with torch.no_grad():
                    trajs += self.algo.create_training_data_from_own_samples(
                        self.model, num_online, cond_info["encoding"][num_offline:], random_action_prob=rp
                    )
            ...
            extra_info = {"random_action_prob": rp}
            if num_online > 0:
            ...
            if self.replay_buffer is not None:
                ...
                if self.rp_push_one:
                    self.replay_buffer.push_many(
                        rewards[num_offline:],
                        trajs[num_offline:],
                        log_rewards[num_offline:],
                        flat_rewards[num_offline:],
                        cond_info[num_offline:],
                        is_valid[num_offline:],
                    )
                else:
                ...
                extra_info["replay_buffer_push_time"] = time.time() - buffer_start_time
                ...
                cond_info = {k: torch.tensor(np.stack([d[k] for d in cond_info])) for k in cond_info[0]}
