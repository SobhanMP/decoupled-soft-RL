...
class MPObjectProxy:
    def __init__(self, obj, num_workers: int, cast_types: tuple, pickle_messages: bool = False, thread=True):
        self.proxy = MPObjectProxy_(obj, num_workers, cast_types, pickle_messages, thread)
        if thread:
            self.thread = threading.Thread(target=self.proxy.run, daemon=True)
        else:
            self.thread = mp.Process(target=self.proxy.run, daemon=True)

        self.keepalive = KeepAlive(self.proxy.stop)
        self.proxy.stop.clear()
        self.thread.start()
        # has to be after thread.start() because it of mp pickling
        self.placeholder = MPObjectPlaceholder(self.proxy.in_queues, self.proxy.out_queues, pickle_messages)


class MPObjectProxy_:
...
def mp_object_wrapper(obj, num_workers, cast_types, pickle_messages: bool = False, thread=False):
    ...
    """
    x = MPObjectProxy(obj, num_workers, cast_types, pickle_messages, thread=thread)
